...
from models import BiDAF,BiDAF_char,QANet,KnowGQA
...
'''
class adamOpt:
    def __init__(self,optimizer, t1,t2,warmup=1000,coolingdown=200000):
    ...
    def rate(self):
        if self.step_count<=self.warmup:
            return self.t1*(self.step_count/self.warmup)**0.5
        elif self.step_count<=self.coolingdown:
            return self.t1
        else:
            return max(self.t2,(1-(self.step_count-self.coolingdown)/self.warmup)**0.5)

def get_opt(model,args):
    return adamOpt(optim.Adam(model.parameters(),lr=1,betas=[0.8,0.999],eps=1e-7,weight_decay=args.l2_wd),args.t1,args.t2)
...
def main(args):
   ...
   # cooldown=args.hidden_size*100
    ...
    if(args.model_name=="KnowGQA"):
        train_dataset = SQUADwithGraph(args.train_record_file_graph, args.use_squad_v2)
        dev_dataset = SQUADwithGraph(args.dev_record_file_graph, args.use_squad_v2)
        ...
        with torch.enable_grad(), \
            ...
            if (args.model_name == "KnowGQA"):
                for cw_idxs, cc_idxs, qw_idxs, qc_idxs,co_idxs,adjs, y1, y2, ids in train_loader:
                    ...
                    # Forward
                    log_p1, log_p2 = model(cw_idxs, cc_idxs, qw_idxs, qc_idxs,co_idxs,adjs)
                    ...
                    if steps_till_eval <= 0:
                        ...
                        ema.assign(model)
                        results, pred_dict = evaluate(model,"KnowGQA", dev_loader, device,
                                                      args.dev_eval_file,
                                                      args.max_ans_len,
                                                      args.use_squad_v2)
                ...
                for cw_idxs,  qw_idxs,  y1, y2, ids in train_loader:
                    ...
                    if steps_till_eval <= 0:
                        ...
                        log.info(f'Evaluating at step {step}...')
                        ema.assign(model)
                        results, pred_dict = evaluate(model,"BiDAF_nochar", dev_loader, device,
                                                      args.dev_eval_file,
                                                      args.max_ans_len,
                                                      args.use_squad_v2)
                        saver.save(step, model, results[args.metric_name], device)
                ...
                for cw_idxs, cc_idxs, qw_idxs, qc_idxs,  y1, y2, ids in train_loader:
                    ...
                    if steps_till_eval <= 0:
                        ...
                        log.info(f'Evaluating at step {step}...')
                        ema.assign(model)
                        results, pred_dict = evaluate(model, "BiDAF", dev_loader, device,
                                                      args.dev_eval_file,
                                                      args.max_ans_len,
                                                      args.use_squad_v2)
                        saver.save(step, model, results[args.metric_name], device)
...
def evaluate(model, model_name,data_loader, device, eval_file, max_len, use_squad_v2):
    ...
    with torch.no_grad(), \
        ...
        if(model_name=="KnowGQA"):
            for cw_idxs, cc_idxs, qw_idxs, qc_idxs, co_idxs,adjs,y1, y2, ids in data_loader:
                # Setup for forward
                cw_idxs = cw_idxs.to(device)
                cc_idxs=cc_idxs.to(device)
                qw_idxs = qw_idxs.to(device)
